...
// require('./crm');   // <-- crm.js will run automatically on port 5001
...
const Violation = mongoose.model(
  "Violation",
  violationSchema,
  "violations"
);
...
app.post("/api/violations", async (req, res) => {
  ...
  try {
    ...
    const exists = await Violation.findOne({
      imageUrl,
      videoFilename
    });
...
app.get("/api/violations/verified/csv", async (req, res) => {
  ...
  try {
    ...
    const data = await Violation.find({ verified: true })
    .sort({ analyzedAt: -1 });
